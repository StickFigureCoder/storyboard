<script lang="ts">
  ...
  const titleKey = $derived(
    field.previewFields?.title ?? subFields.find((f) => f.name === 'title' || f.name === 'name')?.name
  );
...
<div class="space-y-2">
        ...
        <Card.Root class="overflow-hidden p-0">
          <button type="button" class="block w-full text-left" onclick={() => openItem(i)}>
            ...
            <Card.Header class="p-3">
              <Card.Title class="text-sm">{itemLabel(item, i)}</Card.Title>
  ...
  {:else}
    ...
    <Button type="button" variant="outline" size="sm" disabled={items.length >= max} onclick={addItem}>
